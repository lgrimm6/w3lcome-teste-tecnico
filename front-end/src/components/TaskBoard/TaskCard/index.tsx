...
const TaskCard: React.FC<ITaskCardProps> = ({ task }) => {
    const { setCurrentTask, setIsOpenModalEdit, setTasks, tasks } = useContext(TaskContext)
    const [isDelete, setIsDelete] = useState(Boolean)
    const [isCompleted, setIsCompleted] = useState(task.concluida)

    const requestCompletedTask = async (event: React.ChangeEvent<HTMLInputElement>) => {
        try {
            const res = await api.patch<ITask>(`/tasks/${task.id}`, { concluida: event.target.checked })
            const updatedTasks: ITask[] = tasks.map(
                (taskPrev: ITask) => taskPrev.id === task.id ? { ...taskPrev, ...res.data } : taskPrev
            );
    ...
    return (
        <LiStyled className={`${isCompleted ? "completed" : "pending"} ${isDelete ? "delete" : ""}`}>
            <div className="divButtonPine">
                <button className="buttonPine">
                    <BsPinAngleFill size={36} />
                </button>
            </div>
            <div className="conteinerFlex">
                ...
                <div className="containerOptionsTask">
                    ...
                    />
                    <button><RiListSettingsLine size={24} onClick={() => {
                        setIsOpenModalEdit(true)
                        setCurrentTask(task)
                    }} /></button>
                    <button><TfiTrash size={24} onClick={() => requestDeletedTask(task.id)} /></button>
                </div>
            </div>
        </LiStyled>
    )
